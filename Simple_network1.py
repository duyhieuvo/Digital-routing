...
def running(G,events,multiplier,Event,timeline,freePath,freeWavelength,checkBlocking,assignWavelength,Transmission,Release):
    ...
    timeLine = timeline(events,0.1,1*multiplier)
    print timeLine
    ...
    while (True):
        ...
        if (timeLine[i][1]==0):
            ...
            if (checkBlocking(free_Wavelength)):
                ...
                Transmission(wavelength, path, G)
            else:
                assignment.append(0)
                blocking += 1
            k += 1
...
blocking = np.zeros(5)
for i in range(5):
    blocking[i] = running(G,200,i+1,Event,timeline,freePath,freeWavelength,checkBlocking,assignWavelength,Transmission,Release)
print blocking
