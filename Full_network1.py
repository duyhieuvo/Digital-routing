...
#Find paths with specific length
def findPathsNoLC(G,source,length):
    if length==0:
        return [[source]]
    paths = []
    for neighbor in G.neighbors(source):
        for path in findPathsNoLC(G,neighbor,length-1):
            if source not in path:
                paths.append([source]+path)
    return paths

def freePath1(G,event,length):
    paths = findPathsNoLC(G,event[0],length)
    final_paths = []
    for path in paths:
        if(path[length]==event[1]):
...
def running(G,events,multiplier,Event,timeline,freePath,freePath1,freeWavelength,checkBlocking,assignWavelength,Transmission,Release):
    assignment = []
...
blocking = np.zeros(5)
for i in range(5):
    blocking[i] = running(G,10000,i+1,Event,timeline,freePath,freePath1,freeWavelength,checkBlocking,assignWavelength,Transmission,Release)
print blocking
