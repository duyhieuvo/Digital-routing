...
#    def __init__(self, event,wavelength,path,G):
...
#       self.G = G
#    def run(self):
#       Transmission(self.event,self.wavelength,self.path,self.G)
...
def running(event,arrival,multiplier,freePath,freeWavelength,checkBlocking,assignWavelength,Transmission,G):
    ...
    event = event(7000,7*multiplier)
    print event

    arrival= arrival(7000,0.01)
    print arrival
    ...
    while(True):
        ...
        for j in range(len(path)):
            ...
            if (checkBlocking(free_Wavelength)):
                ...
                # thread = myThread(current_event,wavelength, path[j], G)
                # thread.start()
                print blocking
                break
            elif(j==(len(path)-1)):
